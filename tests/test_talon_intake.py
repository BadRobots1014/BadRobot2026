...
from subsystems.talonFXIntake import (
    DUMP_VOLTAGE,
    EXTENSION_VOLTAGE,
    INTAKE_VOLTAGE,
    TalonIntakeSubsystem,
)
...
@pytest.fixture
def talon_intake() -> TalonIntakeSubsystem:
    ...
    subsystem = TalonIntakeSubsystem(intake_motor, right, left, forward, backward)
    subsystem.intake_motor.reset_mock()
    subsystem.left.reset_mock()
    return subsystem


def test_default_voltages(talon_intake: TalonIntakeSubsystem) -> None:
...
def test_set_intake_voltage(talon_intake: TalonIntakeSubsystem) -> None:
    talon_intake.set_intake_voltage(4.5)
    talon_intake.intake_motor.set_voltage.assert_called_once_with(4.5)


def test_set_intake_voltage_from_nt(talon_intake: TalonIntakeSubsystem) -> None:
...
def test_set_dump_voltage_from_nt(talon_intake: TalonIntakeSubsystem) -> None:
