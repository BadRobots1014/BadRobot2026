...
class LightSubsystem(Subsystem):
    def __init__(self, controller: LEDController):
        ...
        self.defult_pattern = self.controller.get_solid(255, 255, 0).atBrightness(BRIGHTNESS_MULTIPLIER)
        self.current_pattern = None

        self.set_default()

    def set_solid(self, r: int, g: int, b: int) -> None:
        self.current_pattern = self.controller.get_solid(r, g, b).atBrightness(BRIGHTNESS_MULTIPLIER)

    def set_rainbow(self, saturation: int, value: int, speed: int) -> None:
        self.current_pattern = self.controller.get_rainbow(saturation, value, speed).atBrightness(BRIGHTNESS_MULTIPLIER)

    def set_gradient(self, continuous: bool, colors: list[tuple]) -> None:
        self.current_pattern = self.controller.get_gradient(continuous, colors).atBrightness(BRIGHTNESS_MULTIPLIER)
